$(document).ready(function () {
  function render(jsonData) {
      ...
      if (
        ...
        if (section.length === 0) {
          const sectionClass = type
            ...
            .toLowerCase();
          const sectionTitle = $(
            `<h2 class="${sectionClass}"><span>${addSToFirstWord(type)}</span></h2>`,
          );
    ...
    catalog.find("ul.section").each(function () {
  ...
  const url =
    "https://sheets.googleapis.com/v4/spreadsheets/1Qz4VqFTEbYCjfHH7WUZ3wFHkfDMPrN05i047muotqtc/values/publicaciones?key=AIzaSyD37ddBLRxw48pq0CLXYd2LIjUrneaKk5s";
