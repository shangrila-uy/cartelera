...
function addLink(element, text, link) {
  element
    .text("")
    .append($("<a>").attr("href", link).attr("target", "_blank").text(text));
}
...
function fillInRow(formattedData, selector, elements) {
  ...
  for (var day in formattedData) {
    var date = new Date(day);
    const dayRow = formattedData[day];
    if (dayRow["type"] !== selector) continue;

    var item = meeting.clone().addClass(dayRow["extra"]);
    if (dayRow["extra"] == "special-talk") {
      item.find(".public-talk > h2").text("DISCURSO ESPECIAL");
    } else if (dayRow["extra"] == "memorial") {
      item.find(".public-talk > h2").text("CONMEMORACIÓN");
    }
    ...
    fillLink(item, dayRow["stream"]);

    const labels = dayRow["labels"];
    const info = dayRow["info"];
    const assignments = dayRow["assignments"];
    const link = dayRow["links"];
...
function readWeekendSheet(weekendMeetingsRequest, today, formattedData) {
  ...
  for (var i = 2; i < weekendMeetingsData.values.length; i++) {
    ...
    if (date >= today && (entry["D".ord()] != "" || entry["E".ord()] != "")) {
      ...
      const isSpecialTalk = entry["H".ord()].includes("Discurso Especial");
      const isMemorial = entry["H".ord()].includes("Conmemoración");
      
      formattedData[date] = {
        type: "weekend",
        extra: isSpecialTalk ? "special-talk" : isMemorial ? "memorial" : "",
        stream: entry["Z".ord()],
        labels: {
          talk: entry["H".ord()].replace("Conmemoración: ", "").replace("Discurso Especial: ", ""),
...
function processData(meetingsRequest, weekendMeetingsRequest) {
  ...
  table.find(".meeting.special + .meeting.special").remove();
  ...
  if (filterData.filterActive) {
    ...
    $(".meeting")
      .filter(function (index) {
        ...
        if (
          $(".row > .assignee", this).length !== filteredRows.length ||
    ...
    setTimeout(function () {
      $(
        ".row:visible .section:visible > *:visible:first-child, .row:visible .assignee:visible:first-child",
      ).addClass("first");
      $(
        ".row:visible .section:visible .assignee:visible, .row:visible .assignee:visible",
      ).addClass("last");
    }, 400);
...
function onFilterClickedListener(filterData) {
  if (filterData.filterActive) {
    ...
    $("#autocomplete").val("");
    $(".friend-list li").show();
    $(".search-field a").addClass("search").text("\uE67D");
    toggleFilterPopup(filterData);
...
function renderAssignee(filterData, assignee) {
  const initials = $("<span>").addClass("initials").text(getInitials(assignee));
  const circle = $("<span>").addClass("circle").append(initials);
  const name = $("<span>").addClass("name").text(assignee);
  const link = $("<a>")
    .attr("href", "#")
    .append(circle)
    .append(name)
...
function initFilters(filterData) {
  const assignmentsUrl =
    "https://sheets.googleapis.com/v4/spreadsheets/1XBm2Ywv2CEr7yHTjfbIHT_vNvHtD0pTX0B8BXmuaGF0/values/Asignaciones?key=AIzaSyD37ddBLRxw48pq0CLXYd2LIjUrneaKk5s";
  ...
  assignmentsRequest.onload = function () {
    ...
    const icon = $("<a>").attr("href", "#").addClass("search").text("\uE67D");
    const close = $("<a>")
      .attr("href", "#")
      .addClass("close")
      .text("\uE6F1")
    ...
    const topbar = $("<div>").addClass("topbar").append(close);
    const searchField = $("<div>")
      .addClass("search-field")
      .append(icon)
      .append(input);
    const ul = $("<ul>");
    const friendList = $("<div>").addClass("friend-list").append(ul);
    const friends = $("<div>")
      .addClass("friends")
      .append(topbar)
      .append(searchField)
    ...
    input.on("change paste keyup", function () {
      ...
      if (value === "") {
        icon.text("\uE67D").addClass("search").off("click");
        ul.find("li").show();
        return;
      }
...
function meetings() {
  var meetingsUrl =
    "https://sheets.googleapis.com/v4/spreadsheets/1XBm2Ywv2CEr7yHTjfbIHT_vNvHtD0pTX0B8BXmuaGF0/values/vida%20y%20ministerio?key=AIzaSyD37ddBLRxw48pq0CLXYd2LIjUrneaKk5s";
  var weekendMeetingsUrl =
    "https://sheets.googleapis.com/v4/spreadsheets/1XBm2Ywv2CEr7yHTjfbIHT_vNvHtD0pTX0B8BXmuaGF0/values/Fin%20de%20semana?key=AIzaSyD37ddBLRxw48pq0CLXYd2LIjUrneaKk5s";
  var filterData = {
    popupVisible: false,
    filterActive: false,
  };
